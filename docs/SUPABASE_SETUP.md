...
10. 在 `/admin/audit` 验证身份变更记录和管理记录（也可使用下方查询）。最后撤销测试权限、删除测试图片、归档两个 DEMO 社团；不要删除真实社团或账号。
...
管理面板尚不是所有数据库字段的通用编辑器：当前网站文字仅支持四个字段；审计页每类显示最新 100 条，完整搜索与部分内容配置仍需继续完善。密码恢复入口已实现但邮件流程仍待实测。
