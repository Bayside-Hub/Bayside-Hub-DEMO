...
export default async function SubmitPage() {
  ...
  return <div className="mx-auto max-w-3xl space-y-6 px-5 py-8"><h1 className="text-3xl font-bold">School announcements</h1><p className="text-muted">Advisors and board members can submit a school-wide announcement for administrator approval.</p>{history.error ? <p role="alert">Submissions are temporarily unavailable. Please contact Support.</p> : clubs?.length ? <SubmissionForm clubs={clubs} /> : <p>No club leadership assignment found. Ask your administrator to assign your club.</p>}<h2 className="text-xl font-semibold">Your submissions</h2>{history.data?.map(row => <article key={row.id} className="rounded-xl border border-line p-5"><h3 className="font-bold">{row.title}</h3><p className="text-sm capitalize">{row.status}</p>{row.review_note && <p>{row.review_note}</p>}</article>)}</div>;
}
