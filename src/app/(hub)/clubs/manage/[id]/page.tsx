...
export default async function ManageClubPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-5 py-8 sm:px-6 lg:py-12">
      ...
      <div id="stream" className="mt-8 grid scroll-mt-28 gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,.75fr)]">
        <main className="space-y-6">
          ...
          <section id="meetings" className="scroll-mt-28 rounded-[20px] border border-line bg-card p-5 shadow-sm sm:p-6">
            ...
            <ActionFeedbackForm action={addClubMeeting} className="mt-5 grid gap-3 sm:grid-cols-2">
              ...
              <label className="text-sm font-medium text-ink">Starts<input type="time" name="start_time" className={`${input} mt-1`} /></label>
              <label className="text-sm font-medium text-ink">Ends<input type="time" name="end_time" className={`${input} mt-1`} /></label>
            ...
            {meetings?.length ? <div className="mt-5 flex flex-wrap gap-2">{meetings.map((meeting) => <span key={meeting.id} className="rounded-full border border-line bg-content-bg px-3 py-1.5 text-xs font-medium text-ink">{["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"][meeting.day_of_week - 1]} · {meeting.start_time?.slice(0, 5) ?? "TBD"} · {meeting.location ?? "TBD"}</span>)}</div> : null}
          </section>
        </main>
        ...
        <aside className="space-y-6">
          <section id="members" className="scroll-mt-28 rounded-[20px] border border-line bg-card p-5 shadow-sm">
            ...
            {canGovern && memberships?.length ? <ul className="mt-4 space-y-3">{memberships.map((membership) => { const profile = profileMap.get(membership.profile_id); return <li key={membership.id} className="rounded-control border border-line p-3"><p className="font-semibold text-ink">{profile?.full_name ?? profile?.email ?? "Student"}</p><p className="mt-0.5 text-xs text-muted">Requested {new Intl.DateTimeFormat("en-US", { dateStyle: "medium" }).format(new Date(membership.requested_at))}</p><div className="mt-3 flex gap-2">{["active", "rejected"].map((status) => <ActionFeedbackForm key={status} action={reviewClubMembership}><input type="hidden" name="club_id" value={club.id} /><input type="hidden" name="membership_id" value={membership.id} /><input type="hidden" name="status" value={status} /><button className={`rounded-full px-4 py-2 text-xs font-semibold ${status === "active" ? "bg-navy text-cream" : "border border-line text-muted"}`}>{status === "active" ? "Approve" : "Decline"}</button></ActionFeedbackForm>)}</div></li>; })}</ul> : <p className="mt-4 text-sm leading-6 text-muted">{canGovern ? "You’re all caught up—there are no pending requests." : "Membership review is available to advisors and staff."}</p>}
          </section>
      ...
      <ClubGovernancePanel clubId={club.id} clubStatus={club.status} role={user.role} canGovern={canGovern} officers={officers ?? []} advisors={advisors ?? []} media={media ?? []} history={history ?? []} compliance={compliance ?? null} />
    </div>
  );
}
