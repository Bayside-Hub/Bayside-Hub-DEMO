...
export default function SiteTextForm({ field, value }: { field: string; value: string }) {
  ...
  return <form action={action} className="grid gap-3 rounded-xl border border-line bg-card p-5 text-ink" aria-busy={pending}>
    ...
    <textarea id={`${descriptionId}-body`} name="body" required maxLength={4000} defaultValue={value} rows={field === "manual_content" ? 14 : 4} aria-describedby={`${descriptionId}-help`} onChange={(event) => setLength(event.currentTarget.value.length)} className="w-full rounded-lg border border-line bg-content-bg p-3 font-normal text-ink" />
    <p id={`${descriptionId}-help`} className="text-xs text-muted">{length}/4000 characters</p>
