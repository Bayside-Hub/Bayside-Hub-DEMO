...
export default function EmailAccount({ next, recovery = false }: { next: string; recovery?: boolean }) {
  ...
  return <section className="login-email mt-5 border-t border-black/10 pt-4 text-[#232b3b]">
    ...
    <form onSubmit={submit} className="grid gap-3.5">
      ...
      <label className="grid gap-1 text-sm">School email<input name="email" type="email" required autoComplete="email" className="min-h-10 rounded-lg border border-black/20 bg-white px-3" /></label>
      {mode !== "recovery" && <label className="grid gap-1 text-sm">Password<input name="password" type="password" required minLength={mode === "signup" ? 12 : 1} maxLength={128} autoComplete={mode === "signup" ? "new-password" : "current-password"} className="min-h-10 rounded-lg border border-black/20 bg-white px-3" /></label>}
